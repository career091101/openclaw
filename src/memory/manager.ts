...
export class MemoryIndexManager {
  ...
  /**
   * Write or append content to a memory file under the workspace `memory/` directory.
   * Path traversal outside `memory/` is rejected. Only `.md` files are allowed.
   * After writing, the file watcher triggers an automatic re-index.
   */
  async writeMemoryFile(params: {
    ...
    // Only allow writing inside memory/ directory (not MEMORY.md root files)
    if (!relPath || relPath.startsWith("..") || path.isAbsolute(relPath)) {
      throw new Error("write restricted to memory/ directory");
    }
    ...
    // Prevent symlink writes
    try {
      const stat = await fs.lstat(absPath);
      if (stat.isSymbolicLink()) {
        throw new Error("cannot write to symlinks");
      }
    } catch (err) {
    ...
    this.dirty = true;
    return { path: relPath, bytesWritten: stat.size };
  }

  /**
   * Delete specific lines from a memory file, or delete the entire file.
   * Path traversal outside `memory/` is rejected.
   */
  async deleteMemoryLines(params: {
    relPath: string;
    fromLine?: number;
    toLine?: number;
  }): Promise<{ path: string; deleted: "lines" | "file"; linesRemoved?: number }> {
    const rawPath = params.relPath.trim();
    if (!rawPath) {
      throw new Error("path required");
    }
    ...
    const relPath = path.relative(this.workspaceDir, absPath).replace(/\\/g, "/");
    if (!relPath || relPath.startsWith("..") || path.isAbsolute(relPath)) {
      throw new Error("delete restricted to memory/ directory");
    }
    if (!relPath.startsWith("memory/")) {
      throw new Error("delete restricted to memory/ directory");
    }
    ...
    // Delete entire file if no line range specified
    if (params.fromLine == null && params.toLine == null) {
    ...
    const from = Math.max(1, params.fromLine ?? 1);
    const to = Math.min(lines.length, params.toLine ?? lines.length);
    if (from > to || from > lines.length) {
      return { path: relPath, deleted: "lines", linesRemoved: 0 };
    }
    const linesRemoved = to - from + 1;
    lines.splice(from - 1, linesRemoved);
    await fs.writeFile(absPath, lines.join("\n"), "utf-8");
    this.dirty = true;
    return { path: relPath, deleted: "lines", linesRemoved };
  }
