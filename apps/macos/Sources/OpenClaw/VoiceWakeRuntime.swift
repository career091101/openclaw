...
actor VoiceWakeRuntime {
    ...
    private func start(with config: RuntimeConfig) async {
        ...
        do {
            // Early-exit when no physical audio input device is connected (e.g. Mac Mini without external mic).
            // Accessing AVAudioEngine.inputNode on such machines can return a ghost format that passes
            // channelCount/sampleRate checks but causes installTap to throw an unrecoverable NSException.
            guard AudioInputDeviceObserver.defaultInputDeviceUID() != nil else {
                self.logger.info("voicewake runtime: no audio input device — skipping start")
                return
            }
            ...
            guard hwFormat.channelCount > 0, hwFormat.sampleRate > 0 else {
                throw NSError(
                    ...
                    code: 1,
                    userInfo: [NSLocalizedDescriptionKey: "No audio input available (format channels=\(hwFormat.channelCount) rate=\(hwFormat.sampleRate))"])
            }
            input.removeTap(onBus: 0)

            // Pass nil format so AVAudioEngine uses the hardware's native format,
            // avoiding "format.sampleRate == hwFormat.sampleRate" assertion failures
            // when the hardware format changes between outputFormat() and installTap().
            // Wrap in ObjC @try/@catch because installTap throws NSException on failure
            // (e.g. duplicate tap, missing mic) which Swift's do/catch cannot intercept.
            // nonisolated(unsafe) is needed to cross the actor boundary into the ObjC block.
            nonisolated(unsafe) let unsafeInput = input
            nonisolated(unsafe) let unsafeRequest = request
            try ObjCExceptionCatcher.perform {
                unsafeInput.installTap(onBus: 0, bufferSize: 2048, format: nil) { [weak self] buffer, _ in
            ...
            // Observe audio configuration changes (device switch, Bluetooth HFP toggle, etc.)
            // so we can restart cleanly instead of crashing on stale engine state.
            self.observeConfigurationChanges(engine: audioEngine)
    ...
    /// Watches for audio hardware changes (device switch, Bluetooth profile toggle, sample rate change)
    /// and restarts the recognition pipeline cleanly.
    private func observeConfigurationChanges(engine: AVAudioEngine) {
        // Remove any previous observer before registering a new one.
        if let old = self.configChangeObserver {
        ...
        let observer = NotificationCenter.default.addObserver(
            ...
            Task { await weakSelf.handleConfigurationChange() }
        }
        self.configChangeObserver = observer
    }

    private func handleConfigurationChange() {
        self.logger.info("voicewake runtime: audio configuration changed, restarting pipeline")
        DiagnosticsFileLog.shared.log(category: "voicewake.runtime", event: "configChange")
        guard let config = self.currentConfig else { return }
        self.haltRecognitionPipeline()
        Task { await self.start(with: config) }
    }
