...
actor TalkModeRuntime {
    ...
    private func startRecognition() async {
        ...
        guard AudioInputDeviceObserver.defaultInputDeviceUID() != nil else {
            self.logger.info("talk mode: no audio input device — skipping start")
            return
        }
        let input = audioEngine.inputNode
        let format = input.outputFormat(forBus: 0)
        guard format.channelCount > 0, format.sampleRate > 0 else {
            self.logger.error("talk mode: invalid audio format (channels=\(format.channelCount) rate=\(format.sampleRate))")
            return
        }
        input.removeTap(onBus: 0)
        let meter = self.rmsMeter
        nonisolated(unsafe) let unsafeInput = input
        nonisolated(unsafe) weak var unsafeRequest = request
        do {
            try ObjCExceptionCatcher.perform {
                unsafeInput.installTap(onBus: 0, bufferSize: 2048, format: nil) { [meter] buffer, _ in
            ...
            self.logger.error("talk mode: installTap failed: \(error.localizedDescription, privacy: .public)")
            return
        }
