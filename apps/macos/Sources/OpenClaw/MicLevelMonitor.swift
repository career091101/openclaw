...
actor MicLevelMonitor {
    ...
    func start(onLevel: @Sendable @escaping (Double) -> Void) async throws {
        ...
        // Bail out early when no physical input device exists (e.g. Mac Mini without external mic).
        ...
        // Use nil format to let AVAudioEngine pick the hardware's native format,
        // and wrap in ObjC @try/@catch to survive NSException from installTap.
        // nonisolated(unsafe) is required because ObjCExceptionCatcher.perform crosses
        // the actor isolation boundary (ObjC block cannot be actor-isolated).
        nonisolated(unsafe) let unsafeInput = input
        do {
            try ObjCExceptionCatcher.perform {
                unsafeInput.installTap(onBus: 0, bufferSize: 512, format: nil) { [weak self] buffer, _ in
